...
type RestOf<T extends ISale | NewSale> = Omit<T, 'fechaVenta'> & {
...
export class SaleService {
  ...
  protected convertDateFromClient<T extends ISale | NewSale | PartialUpdateSale>(sale: T): RestOf<T> {
    return {
      ...sale,
      fechaVenta: sale.fechaVenta?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSale: RestSale): ISale {
    return {
      ...restSale,
      fechaVenta: restSale.fechaVenta ? dayjs(restSale.fechaVenta) : undefined,
    };
  }
